...
// Plantilla HTML para el correo que manda lib/mailer.ts. Usa tablas y
...
// mayoría de los clientes de correo). Mismo patrón que
// lib/email-templates.ts en yelifin-sistema, con la marca de Partners
// (--primary de globals.css) y copy para coordinadores de incubadora.

const BRAND_BLUE = "#1455F6";
const TEXT_DARK = "#17181C";
const TEXT_MUTED = "#6C7280";
const BORDER = "#E7E9EE";
const BG = "#F1F3F6";
          ...
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="width:480px; max-width:100%; background-color:#ffffff; border:1px solid ${BORDER}; border-radius:16px;">
            <tr>
              <td style="padding:36px 32px 8px 32px;">
                <!-- Logo: cajita azul "hi" + wordmark, igual que components/shared/hikonta-icon.tsx -->
                <table role="presentation" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td style="padding-left:8px; color:${TEXT_DARK}; font-weight:700; font-size:18px;">HiKonta Partners</td>
                  </tr>
                </table>
              </td>
            </tr>
          ...
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="width:480px; max-width:100%;">
            <tr>
              <td style="padding:20px 32px 0 32px; color:${TEXT_MUTED}; font-size:12px; line-height:1.6; text-align:center;">
                © ${new Date().getFullYear()} HiKonta Partners. Parte del ecosistema HiKonta.
              </td>
            </tr>
          </table>
        </td>
...
export function verifyPartnerEmailTemplate({
  actionLink,
  incubatorName,
}: {
  actionLink: string;
  ...
  const bodyHtml = `
    ...
    <p style="margin:0; color:${TEXT_MUTED}; font-size:15px; line-height:1.6;">
      Gracias por registrar <strong style="color:${TEXT_DARK};">${incubatorName}</strong> en HiKonta Partners.
      Confirma tu correo para activar tu cuenta — un coordinador de HiKonta todavía tiene que
      aprobar el acceso y vincular tu portafolio antes de que veas datos en el panel.
    </p>
  ...
  const text = [
    "Verifica tu correo — HiKonta Partners",
    "",
    `Gracias por registrar ${incubatorName} en HiKonta Partners. Confirma tu correo para activar tu cuenta — un coordinador de HiKonta todavía tiene que aprobar el acceso y vincular tu portafolio antes de que veas datos en el panel.`,
    "",
  ...
  return {
    subject: "Verifica tu correo — HiKonta Partners",
    html: emailShell({ previewText: "Confirma tu correo para activar tu cuenta de HiKonta Partners.", bodyHtml }),
    text,
  };
}

export function resetPartnerPasswordTemplate({ actionLink }: { actionLink: string }): {
  subject: string;
  html: string;
  text: string;
} {
  const bodyHtml = `
    ...
    <p style="margin:0; color:${TEXT_MUTED}; font-size:15px; line-height:1.6;">
      Recibimos una solicitud para restablecer la contraseña de tu cuenta de HiKonta Partners.
      Crea una nueva contraseña desde el siguiente botón.
    </p>
  ...
  const text = [
    "Restablece tu contraseña — HiKonta Partners",
    "",
    "Recibimos una solicitud para restablecer la contraseña de tu cuenta de HiKonta Partners. Crea una nueva contraseña desde el siguiente enlace.",
    "",
  ...
  return {
    subject: "Restablece tu contraseña — HiKonta Partners",
    html: emailShell({ previewText: "Crea una nueva contraseña para tu cuenta de HiKonta Partners.", bodyHtml }),
    text,
  };
}
